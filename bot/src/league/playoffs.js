...
export function advancePlayIn(games) {
  ...
  while (changed) {
    ...
    for (let i = 0; i < winners.length; i += 2) {
      newGames.push({
        id: uid('g'),
        date: `Play-In R${maxRound + 1}`,
        isPlayIn: true,
        playInRound: maxRound + 1,
        bracketSlot: i / 2,
...
function buildNextRound(winners, round) {
  const newGames = [];
  for (let i = 0; i < winners.length; i += 2) {
    newGames.push({
      id: uid('g'),
      date: `Playoffs R${round + 1}`,
      isPlayoff: true,
      playoffRound: round + 1,
      bracketSlot: i / 2,
      seriesGame: 1,
      higherSeedId: winners[i],
      awayTeamId: winners[i + 1],
      homeTeamId: winners[i],
      awayScheduleName: null,
      homeScheduleName: null,
    ...
    for (const round of playoffRoundNums) {
      ...
      for (const slot of slots) {
        ...
        if (slotGames.length === 1 && slotGames[0].isBye) {
          slotWinner[slot] = slotGames[0].homeTeamId;
          continue;
        }
        ...
        if (!hasNextGame && gamesPlayed > 0 && gamesPlayed < sl) {
          ...
          const ids = [...teamIds];
          const lowerSeedId = ids[0] === higherSeedId ? ids[1] : ids[0];
        ...
        if (winners.length >= 2) {
          if (settings.reseedPlayoffs && seedById) {
            winners = [...winners].sort((a, b) => {
              const sa = seedById[a] == null ? 999 : seedById[a];
              const sb = seedById[b] == null ? 999 : seedById[b];
              return sa - sb;
            });
            const paired = [];
            for (let i = 0, j = winners.length - 1; i < j; i += 1, j -= 1) paired.push(winners[i], winners[j]);
            winners = paired;
          }
          if (!nextRoundExists) {
            result = [...result, ...buildNextRound(winners, round)];
            changed = true;
            break;
          }
          ...
          nextSlots.forEach(ns => {
            const expectedA = winners[ns * 2];
            const expectedB = winners[ns * 2 + 1];
            const slotGames = nextRoundGames.filter(g => g.bracketSlot === ns);
            const actualIds = new Set();
            slotGames.forEach(g => {
              if (g.homeTeamId) actualIds.add(g.homeTeamId);
              if (g.awayTeamId) actualIds.add(g.awayTeamId);
            });
...
export function seriesWinsFor(games, round, bracketSlot) {
  const slotGames = games.filter(g => g.isPlayoff && g.playoffRound === round && g.bracketSlot === bracketSlot);
  const wins = {};
  slotGames.forEach(g => {
    if (!g.played) return;
    const w = gameWinner(g);
